...
const INPUTS = {
  ...
  newPassword:
    process.env.ANCHOR_WEATHER_NEW_PASSWORD || process.env.ANCHOR_NEW_PASSWORD,
  currentPassword:
    process.env.ANCHOR_WEATHER_CURRENT_PASSWORD ||
    process.env.ANCHOR_CURRENT_PASSWORD ||
    "", // falls back to login password below
};
...
async function gotoWithLoad(page: any, url: string, timeout = 45000) {
  try {
    ...
    if (atUrl.includes(new URL(url).hostname)) {
      console.warn(
        `[nav] Load timeout, but currently at ${atUrl}. Continuing.`
      );
    } else {
      throw err;
    }
...
async function clickWithRetry(page: any, selector: string, attempts = 2) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      lastErr = e;
      console.warn(
        `[clickWithRetry] Attempt ${i + 1} failed for ${selector}. Retrying...`
      );
...
export default async function WeatherComChangePassword() {
  ...
  if (!email || !loginPassword || !newPassword) {
    const msg =
      "Missing required inputs: ANCHOR_WEATHER_EMAIL/ANCHOR_USERNAME, ANCHOR_WEATHER_PASSWORD/ANCHOR_PASSWORD, ANCHOR_WEATHER_NEW_PASSWORD/ANCHOR_NEW_PASSWORD";
    console.error(msg);
  ...
  try {
    ...
    if (!(await isLoggedIn(page))) {
      ...
      await clickWithRetry(
        page,
        'button.Button--primary--I3yI4.MemberLoginForm--submitButton--Bz-ob[type="submit"]'
      );
      ...
      try {
        ...
        if (atUrl.includes("weather.com")) {
          console.warn(
            `[login] Load wait timed out but at ${atUrl}. Proceeding.`
          );
        } else {
          throw e;
        }
    ...
    await waitVisible(
      page,
      "button.MemberProfileForm--changePasswordButton--EzOT4"
    );
    ...
    await clickWithRetry(
      page,
      'button.Button--primary--I3yI4.MemberChangePasswordForm--submitButton--9cU6R[type="submit"]'
    );

    // Step G: Verify success - wait for modal inputs to disappear (dialog closes)
    console.log(
      "[Step G] Verify password change by waiting for dialog to close"
    );
    let success = false;
    try {
      await page
        .locator("#changePasswordCurrentPassword")
    ...
    if (!success) {
      ...
      const errorText = await page
        .locator('[class*="Error"], [data-error], .FormError')
        .first()
        .innerText()
        .catch(() => "Unknown error");
