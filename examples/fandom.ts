...
const SEL = {
  ...
  signinUsername:
    'input#identifier.wds-input__field[data-test="signin-username-field"]',
  signinPassword:
    'input#password.wds-input__field[data-test="signin-password-field"]',
  ...
  settingsNewPassword:
    'input#password.wds-input__field[data-test="settings-password-field"]',
...
function buildUrls(baseUrl: string) {
  return {
    ...
    changePasswordUrl: `${baseUrl.replace(
      /\/$/,
      ""
    )}/wiki/Special:ChangePassword`,
...
export default async function FandomChangePasswordFlow() {
  ...
  if (!username || !password || !newPassword) {
    const msg =
      "Missing required inputs: ANCHOR_FANDOM_USERNAME, ANCHOR_FANDOM_PASSWORD, ANCHOR_FANDOM_NEW_PASSWORD";
    console.error(msg);
  ...
  try {
    ...
    if (!page.url().includes("community.fandom.com")) {
      console.log(
        "Post-login URL is not on community.fandom.com; proceeding anyway as redirects may follow."
      );
    }
    ...
    console.log(
      "Waiting for settings new password field on Auth Settings page"
    );
    ...
    try {
      await page.waitForSelector(SEL.signinUsername, {
        state: "visible",
        timeout: 6000,
      });
      console.log(
        "Re-authentication detected after password change. Signing in with NEW password."
      );
      await signIn(page, username, newPassword);
    } catch {
      console.log(
        "No re-authentication form detected after password change. Proceeding to logout."
      );
    }
...
// FandomChangePasswordFlow().then((result) => {
//   console.log(result);
// });
