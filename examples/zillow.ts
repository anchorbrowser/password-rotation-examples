...
const INPUTS = {
  ...
  startUrl:
    process.env.ANCHOR_START_URL ||
    "https://www.zillow.com/auth/user/login?entry_point=auth_ui_service_error_page&prompt=login",
  profileUrl:
    process.env.ANCHOR_PROFILE_URL ||
    "https://www.zillow.com/myzillow/profile/",
  logoutUrl:
    process.env.ANCHOR_LOGOUT_URL || "https://www.zillow.com/Logout.htm",
};
...
async function safeGoto(
  ...
  try {
    ...
    if (!resp) {
      console.log(
        `[NAV] ${label}: no response object returned (may still be fine).`
      );
      return true;
    }
    if (!resp.ok()) {
      console.error(
        `[NAV] ${label}: HTTP ${resp.status()} ${resp.statusText()}`
      );
    }
    return true;
  } catch (err: any) {
    ...
    try {
      ...
      if (targetHost && currentHost && targetHost === currentHost) {
        console.log(
          `[NAV] ${label}: load timeout/warning but on expected host ${currentHost}. Continuing.`
        );
        return true;
      }
    } catch {}
    console.error(
      `[NAV] ${label}: failed to navigate -> ${err?.message || err}`
    );
    return false;
  }
}
...
async function expectVisible(locator: any, desc: string, timeout = 15000) {
  try {
    ...
    console.error(
      `[WAIT] ${desc}: not visible within ${timeout}ms -> ${
        err?.message || err
      }`
    );
    return false;
...
export default async function zillowChangePasswordFlow() {
  ...
  try {
    ...
    const emailInput = page
      .locator('input[data-testid="identifier-input"]')
      .first();
    const emailReady = await expectVisible(
      emailInput,
      'Email input [data-testid="identifier-input"]'
    );
    if (!emailReady)
      return { success: false, message: "Email input not available" };

    console.log(
      `[ACTION] Typing email (${maskSecret(INPUTS.email)}) and submitting`
    );
    ...
    if (!pwdReady)
      return { success: false, message: "Password input not available" };

    console.log(
      `[ACTION] Typing current password (${maskSecret(
    ...
    if (!ok2)
      return { success: false, message: "Failed to open My Profile page" };

    // Step 5 (Recorded element: button[aria-label="Change password"]) open dialog
    const changePwdBtn = page
      .locator('button[aria-label="Change password"]')
      .first();
    const changeBtnReady = await expectVisible(
      changePwdBtn,
      'Change password button [aria-label="Change password"]'
    );
    if (!changeBtnReady)
      return {
        success: false,
        message: "Change password button not available",
      };
    ...
    if (!dlgInputsReady)
      return {
        success: false,
        message: "One or more password dialog inputs are not available",
      };

    console.log(
      `[ACTION] Filling current (${maskSecret(
        INPUTS.currentPassword
      )}), new (${maskSecret(INPUTS.newPassword)}), and confirm new password`
    );
    ...
    const dialogForm = page
      .locator("form")
      .filter({ has: currentPwdInput })
      .first();
    const applyBtn = dialogForm.locator('button[type="submit"]').first();
    const applyReady = await expectVisible(
      applyBtn,
      "Apply button (type=submit) inside dialog"
    );
    if (!applyReady)
      return { success: false, message: "Apply button not available" };
    ...
    if (!ok3) {
      console.error(
        "Logout navigation experienced an issue, but continuing to finalize."
      );
    }

    const outputMessage =
      "Password change flow executed and logout attempted successfully.";
