...
const INPUTS = {
  loginUrl: process.env.ANCHOR_LOGIN_URL || 'https://pcpartpicker.com/accounts/login/',
  changePasswordUrl: process.env.ANCHOR_CHANGE_PASSWORD_URL || 'https://pcpartpicker.com/accounts/password/change/',
  // Accept common fallbacks while retaining ANCHOR_ prefix
  username: process.env.ANCHOR_USERNAME || process.env.ANCHOR_LOGIN || process.env.ANCHOR_EMAIL,
...
async function elementExists(page: any, selector: string) {
  return (await page.locator(selector).count()) > 0;
}
function listMissingInputs(): string[] {
  const missing: string[] = [];
  if (!INPUTS.username) missing.push('ANCHOR_USERNAME (or ANCHOR_LOGIN / ANCHOR_EMAIL)');
  if (!INPUTS.oldPassword) missing.push('ANCHOR_PASSWORD (or ANCHOR_OLD_PASSWORD)');
...
export default async function pcppChangePasswordTask() {
  ...
  try {
    ...
    const submitSel = '#form_submit, form[action="/accounts/login/"] button[type="submit"], form[action="/accounts/login/"] input[type="submit"]';
    ...
    await page.locator(userSel).first().fill(INPUTS.username);
    console.log('[action] fill password');
    await page.locator(passSel).first().fill(INPUTS.oldPassword);
    console.log('[action] submit login');
    await Promise.all([
      page.waitForLoadState('load', { timeout: 25000 }).catch(err => console.error(`[warn] load wait after login: ${String(err)}`)),
      page.locator(submitSel).first().click(),
    ]);
    // Verify login success (look for logout link or account menu)
    const loggedIn = await page.locator('a[href="/accounts/logout/"]').first().isVisible().catch(() => false);
    const stillOnLogin = await page.locator(loginFormSel).first().isVisible().catch(() => false);
    console.log(`[state] After login: url=${page.url()} loggedIn=${loggedIn} stillOnLogin=${stillOnLogin}`);
    if (!loggedIn && stillOnLogin) {
      const msg = 'Login appears to have failed: login form still visible after submit.';
      console.error(msg);
      return { success: false, message: msg };
    }
    ...
    const newPass1Sel = '#id_new_password1, input[name="new_password1"], input[name="new_password"]';
    const newPass2Sel = '#id_new_password2, input[name="new_password2"], input[name="new_password_confirm"]';
    ...
    await page.locator(oldPassSel).first().fill(INPUTS.oldPassword);
    console.log('[action] fill new password');
    await page.locator(newPass1Sel).first().fill(INPUTS.newPassword);
    console.log('[action] confirm new password');
    await page.locator(newPass2Sel).first().fill(INPUTS.newPassword);
    console.log('[action] submit change password');
    const changeBtnSel = 'input.button[type="submit"][value="Change Password"], form[action*="/accounts/password/change/"] [type="submit"]';
    await Promise.all([
      page.waitForLoadState('load', { timeout: 25000 }).catch(err => console.error(`[warn] load wait after change submit: ${String(err)}`)),
      page.locator(changeBtnSel).first().click(),
    ]);
    // Step 3: Verify success
    const url = page.url();
    const successUrl = url.includes('/accounts/password/change/done/');
    const successTextVisible = await page.locator('text=Your password has been changed.').first().isVisible().catch(() => false);
    if (successUrl || successTextVisible) {
      const msg = 'Password changed successfully.';
      console.info(msg);
      return { success: true, message: msg };
    }
    const msg = 'Password change may have failed: success URL or confirmation text not detected.';
    console.error(msg);
    return { success: false, message: msg };
  } catch (error) {
